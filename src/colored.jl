function normal_clusters(nx, ny; dim=2)
    x0 = poisson(nx; dim)
    λ = ny / nx
    y0 = clusters(x0, () -> normal(rand(Poisson(λ)); dim) / λ)
    x0, y0
end
...
function colored_clusters(nsup, npp)
    dim = 2
    z = poisson(4nsup; dim)
    ...
    ys = z[:, 2:2:end]
    λ = npp / nsup
    x0 = clusters(xs, () -> normal(rand(Poisson(λ)); dim) / λ)
    y0 = clusters(ys, () -> normal(rand(Poisson(λ)); dim) / λ)
    x0, y0
end
